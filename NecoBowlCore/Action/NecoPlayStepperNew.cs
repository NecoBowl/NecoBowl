...
internal class NecoPlayStepperNew
{
    ...
    public IEnumerable<NecoPlayfieldMutation> Process()
    {
        ...
        while (Movements.Any(m => m.Value.IsChange) || PendingMutations.Any()) {
            ...
            foreach (var (uid, movement) in unitBuffer.Select(kv => (kv.Key, kv.Value))) {
                ...
                if (collisionError.Any()) throw new NecoPlayfieldMutationException("collision in the unit buffer");
        ...
        foreach (var entry in MutationHistory) Logger.Debug(entry);
    ...
    private void ConsumeMutations()
    {
        ...
        foreach (var mutation in PendingMutations) tempMutations.AddRange(mutation.AddMutations(Field.AsReadOnly()));
    ...
    private void FixOutOfBoundsMovements()
    {
        ...
        foreach (var (id, movement) in Movements.Where(m
                     => m.Value.Source?.ResultKind == NecoUnitActionResult.Kind.Failure))
