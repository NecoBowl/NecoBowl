...
internal class NecoPush : INecoPushInformation
{
    ...
    public void FinishTurn()
    {
        ...
        foreach (var (role, plan) in Plans) plan.AddCardPlays(CurrentTurn.CardPlaysByRole[role]);
    }
        ...
        foreach (var (role, plan) in Plans) {
            ...
            foreach (var cardPlay in plays)
                if (cardPlay.Card.IsUnitCard(out var unitCard))
                    field[cardPlay.Position]
                        = field[cardPlay.Position] with { Unit = unitCard!.ToUnit(cardPlay.Player) };
        }
